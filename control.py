...
==============================================================================
'''
# Populated by _initialize(). Kept as module globals because the helper
# functions below (and the exit handler) reference them directly.
settings = None
control = None
errors = None
pelletdb = None
controlLogger = logging.getLogger('control')
...
def _initialize():
	'''
	Read settings, set up logging, flush the Redis structures and load the
	platform / probe / display / distance modules. This is the code that used
	to run at import time; the bodies are unchanged.
	'''
	global settings, control, errors, pelletdb, controlLogger, eventLogger, units, platform_config
	global grill_platform, probe_complex, display_device, dist_device, GrillPlatModule, DisplayModule, DistanceModule
...
def _work_cycle(mode, grill_platform, probe_complex, display_device, dist_device):
	...
	monitor = Process_Monitor('control', ['supervisorctl', 'restart', 'fireai-control'], timeout=30)
...
def main(stop_event=None):
	'''
	Control process entry point.

	:param stop_event: optional threading.Event; when set, the main loop exits
	                   after the current iteration. Used by the test-suite and
	                   the development harness. Production runs with None.
	'''
	global settings, control, errors, pelletdb
	_initialize()
